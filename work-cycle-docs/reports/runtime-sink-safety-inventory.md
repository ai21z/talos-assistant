...
|---|---|---|---|---|---|
| SLF4J/logback file logs | Runtime and provider call sites | `SafeLogFormatter`, `ProtectedContentPolicy.sanitizeText` | `SensitiveLogRedactionTest`, `EmbeddingsClientDiagnosticTest`, `ProcessCommandRunnerTest` | Focused T283 provider/backend installed-product log scan passed for `t283-installed-live-20260520-215141-r2`; focused T283 command-profile installed-product log scan passed for `t283-command-profile-20260520-220959` | Broader two-model prompt-bank evidence still needs log capture review |
...
scanned rebaseline. The remaining release blocker is narrower: produce lane-labeled
two-model prompt-bank evidence, with approval-sensitive cases routed through a
synchronized/manual lane rather than blind redirected stdin.
