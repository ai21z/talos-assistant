...
- `default-config.yaml` excludes `.env`, `.env.*`, `*.env`, `secrets/**`, `.ssh/**`, `.aws/**`, `.azure/**`, `.gnupg/**`, `.config/gcloud/**`, `protected/**`, PDF/Office/image/archive/binary families, plus extra repo/build folders such as `.vscode`, `.external assistant`, `.gradle`, `.mvn`, `node_modules`, `dist`, `prompts`, and `META-INF`.
...
14. Was `alex000kim-article.txt` present in the repo? No. Recursive search for `alex000kim-article.txt`, `local coding assistant Source Leak`, `KAIROS`, `bashSecurity`, and `promptCacheBreakDetection` only found prior report/ticket notes.
