...
|---|----------|------|-----------|
| R1 | `.external assistant/external-coding-assistant/` | Reverse-engineered source (TypeScript, ~1900 files) | model provider local coding assistant, from the March 2026 source-map leak |
| R2 | `.external assistant/gemini-cli/` | Official OSS source (TypeScript monorepo) | Google Gemini CLI |
| R3 | `.external assistant/hermes-agent/` | OSS source (Python) | Hermes agent harness |
| R4 | `.external assistant/openclaw/` | OSS source (TypeScript monorepo, ~18k files) | OpenClaw ("the AI that actually does things") |
| B1 | `.external assistant/Build_an_AI_Agent_(From_Scratch)_v5_MEAP.pdf` | Manning MEAP book | Single-agent, context-engineering focused |
| B2 | `.external assistant/Build_a_Multi-Agent_System_(MEAP-Book).pdf` | Manning MEAP book | Multi-agent orchestration |
| A1 | `.external assistant/alex000kim-article (1).txt` | Article | Analysis of the local coding assistant source leak |
...
|---|---|---|
| local coding assistant | ripgrep-backed `Grep`, `Glob`, `Read`; open-ended search delegated to a sub-agent | R1 `src/tools/GrepTool/prompt.ts:7-17` ("A powerful search tool built on ripgrep"), `src/tools/GlobTool/GlobTool.ts:57-89`, `src/tools/AgentTool/built-in/exploreAgent.ts` ("EXCLUSIVELY to search and analyze existing code") |
...
- B1 (From Scratch), §5.1.2: *"Tools like local coding assistant, Cursor, and Gemini CLI understand code in
...
|---|---|---|
| local coding assistant | `external assistant.md` hierarchy: managed → user (`~/.external assistant`) → project → local; `@include` expansion; recommended max 40k chars | R1 `src/utils/external assistantmd.ts:1-26, 18-25, 91-93, 618-685` |
...
|---|---|---|
| local coding assistant | autoCompact at `effectiveWindow − 13_000` buffer; manual at `−3_000`; **`MAX_CONSECUTIVE_AUTOCOMPACT_FAILURES = 3`** circuit breaker (resets on success) | R1 `src/services/compact/autoCompact.ts:62-70, 72-91, 257-349` |
...
This is the theme Talos most under-weights, and it is everywhere in the strongest system (local coding assistant):
...
|---|---|---|
| local coding assistant | rules → allow/deny/ask; `dontAsk` turns ask→deny; auto-mode **classifier** with a safe-tool allowlist fast-path; 23 numbered bash security checks | R1 `src/utils/permissions/permissions.ts:122-231, 473-517, 658-760`; A1 line 87 |
...
- local coding assistant's multi-agent coordinator logic is *entirely in a system prompt*: *"Do not rubber-stamp
...
stores, (3) = external assistant.md/MEMORY.md + dream/distillation.
...
2. **Add a compaction loop with the reference rules (C3, F3-layer-1).**
   ...
   (Gemini's Probe), and a **`MAX_CONSECUTIVE_*_FAILURES` circuit breaker** (local coding assistant's 3-strike
...
| ID | Path |
|----|------|
| R1 | `.external assistant/external-coding-assistant/src/...` (GrepTool, GlobTool, AgentTool, coordinatorMode, autoCompact, permissions, external assistantmd, systemPromptSections, bootstrap/state, context) |
