# [T941-in-progress-medium] Cloudflare Pages staging deploy workflow
...
- Talos version / commit: 0.10.8 /
  897746f712ae495312ddbc22240554e231ebf561
- Branch: `v0.9.0-beta-dev`
- Evidence:
  - Cloudflare Pages project `taloslocal` exists.
  - Project has `source: null`, so no Git auto-deploy is connected.
  - Project has no deployment yet and only the `taloslocal.pages.dev` domain.
...
- Verified `CLOUDFLARE_API_TOKEN` is not present in GitHub secrets, so hosted
  deployment remains blocked until the owner creates a scoped Cloudflare token
  and stores it as that secret.
...
```text
GitHub secret CLOUDFLARE_API_TOKEN is not configured.
```

Do not close this ticket until the secret exists, the hosted
`site-staging.yml` workflow runs green against the intended SHA, and the
resulting `site-staging.taloslocal.pages.dev` preview is inspected.
