# Talos documentation

Talos is a local-first CLI workspace assistant for developer workspaces. It can inspect supported local files, ask before risky changes, run bounded commands through configured profiles, and keep local trace evidence for review.

Start here:

1. [Installation](getting-started/installation.md)
2. [Windows setup](getting-started/windows-setup.md) or [Linux setup](getting-started/linux-setup.md)
3. [Quickstart](getting-started/quickstart.md)
4. [First run](getting-started/first-run.md)
5. [Model setup](getting-started/model-setup.md)

Main references:

- [Commands](user/commands.md)
- [Modes](user/modes.md)
- [Permissions and approvals](user/permissions-and-approvals.md)
- [Privacy and artifacts](user/privacy-and-artifacts.md)
- [Supported files](user/supported-files.md)
- [Troubleshooting](user/troubleshooting.md)
- [CLI reference](reference/cli.md)
- [Configuration](reference/config.md)
- [Model profiles](reference/model-profiles.md)
- [Release channels](reference/release-channels.md)

Architecture and development:

- [Architecture overview](architecture/overview.md)
- [Execution model](architecture/execution-model.md)
- [Trust boundaries](architecture/trust-boundaries.md)
- [Package map](architecture/package-map.md)
- [Developer setup](development/developer-setup.md)
- [Build and test](development/build-and-test.md)
- [CI/CD](development/ci-cd.md)
- [Quality reports](development/quality-reports.md)
- [Release process](development/release-process.md)
- [Runtime artifacts](development/runtime-artifacts.md)
