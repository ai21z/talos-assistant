...
- local OpenClaw source under `.external assistant/openclaw`
- local MEAP PDF under `.external assistant/Build_a_Multi-Agent_System(MEAP-Book).pdf`
- local Alex Kim article under `.external assistant/alex000kim-article (1).txt`
- official OpenAI, Gemini CLI, local coding assistant, Codex, and Terminal-Bench sources
...
git -C .external assistant\openclaw status -sb
git -C .external assistant\openclaw rev-parse HEAD
```
...
| local coding assistant settings | https://docs.external assistant.com/en/docs/external-coding-assistant/settings | public docs | scopes, settings hierarchy, sensitive file examples | Settings and policy comparison |
| local coding assistant permissions | https://code.external assistant.com/docs/en/permissions | public docs | deny -> ask -> allow precedence | Permission precedence comparison |
| local coding assistant hooks | https://docs.external assistant.com/en/docs/external-coding-assistant/hooks | public docs | hook lifecycle and policy integration concepts | Hook comparison, deferred |
...
| Local OpenClaw | `.external assistant/openclaw` | `main`, `a093b5b2de98bf8f18ddda919aa539c7f53d3791` | `docs/plugins/architecture.md`, `src/plugin-sdk/provider-tools.ts`, `src/context-engine/types.ts`, `src/plugin-sdk/plugin-entry.ts`, command registry files | Capability/registry/context comparison |
| MEAP agent source | `.external assistant/Build_a_Multi-Agent_System(MEAP-Book).pdf` | local PDF | pages around agent definition, tool call loop, planning loop | Agent fundamentals |
| Alex Kim article | `.external assistant/alex000kim-article (1).txt` | local text | whole article | Conceptual product-pattern reference only |

Unavailable or not found locally:

- No separate local `opencode`, `OpenCode`, `claw-code`, `ClawCode`, or
  `collection-external-coding-assistant-source-code` source was found under this repo
  workspace beyond `.external assistant/openclaw`.
...
### local coding assistant Official Docs

Sources:

- https://docs.external assistant.com/en/docs/external-coding-assistant/settings
- https://code.external assistant.com/docs/en/permissions
- https://docs.external assistant.com/en/docs/external-coding-assistant/hooks
...
- `.external assistant/openclaw`
- branch `main`
- commit `a093b5b2de98bf8f18ddda919aa539c7f53d3791`

Files inspected:

- `.external assistant/openclaw/docs/plugins/architecture.md`
- `.external assistant/openclaw/src/plugin-sdk/plugin-entry.ts`
- `.external assistant/openclaw/src/plugin-sdk/provider-tools.ts`
- `.external assistant/openclaw/src/context-engine/types.ts`
- command registry files under `.external assistant/openclaw/src/auto-reply`
...
### local coding assistant Leak Article / Mirrored Code

Local source:

- `.external assistant/alex000kim-article (1).txt`

Use status:

- Conceptual/product-pattern reference only.
- Not official model provider documentation.
...
- `.external assistant/Build_a_Multi-Agent_System(MEAP-Book).pdf`
...
| Allow/ask/deny | local coding assistant docs, Talos T35 | Already correct direction | Keep | Deny-first policy aligns with local trust |
...
| Project instruction files | Codex/Gemini/external assistant patterns | Useful but risky with untrusted workspace | Defer | Needs trusted folder and prompt audit first |
