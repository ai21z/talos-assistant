param(
  ...
  [switch]$IncludeFiles,          # add -IncludeFiles to list files too
  [int]$MaxDepth = [int]::MaxValue,
  [switch]$Unicode                # add -Unicode for ├── style branches
)
...
# Optional Unicode (built from code points to avoid mojibake)
if ($Unicode) {
...
function Write-Tree([string]$dir, [string]$prefix, [int]$depth) {
  ...
  # Safe concatenation (works even when one side is a single object)
  $items = @()
  if ($dirs)  { $items += @($dirs) }
...
Write-Host "Wrote tree to $Output"
  ...
  for ($i = 0; $i -lt $items.Count; $i++) {
    ...
    if ($items[$i].PSIsContainer) {
      $nextPrefix = if ($isLast) { $prefix + $space } else { $prefix + $pipe }
      $lines += Write-Tree -dir $items[$i].FullName -prefix $nextPrefix -depth ($depth + 1)
    }
  }
  return $lines
}
...
$lines = @()
$lines += $Root
$lines += Write-Tree -dir $Root -prefix '' -depth 1
